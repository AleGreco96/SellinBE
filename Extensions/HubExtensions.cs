using ChatLib.Services;
using ErrorLib.Utilities;
using Microsoft.AspNetCore.SignalR;
using System.Text;

namespace SellinBE.Extensions
{
    public class HubExtensions : Hub
    {
        private readonly MessageService _messageService;

        public HubExtensions(MessageService messageService)
        {
        ...
        public async Task RequestResponse(string user, string message)
        {
            ...
            {
                var traceId = Context.ConnectionId;

                ErrorDispatcher.DispatchException(
                    ex,
                    traceId,
                    source: nameof(HubExtensions)
                );
